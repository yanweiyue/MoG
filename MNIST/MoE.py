...
class MoE(nn.Module):
    ...
    def __init__(self, in_dim,emb_dim,hidden_size, num_experts,nlayers,activation,k_list,expert_select ,noisy_gating=True, coef=1e-2,edge_dim=1,lam=0.1):
        ...
        self.num_experts = num_experts
        # instantiate experts
        self.experts = nn.ModuleList([SpLearner(nlayers=nlayers, in_dim=in_dim*2+edge_dim, hidden=hidden_size, activation=activation, k=k) for k in k_list])
    ...
    def forward(self, x, edge_index, temp, edge_attr=None, training=False):
        ...
        # edge_gates = edge_gates + torch.rand(edge_gates.size(),device=edge_gates.device)*edge_gates.mean()/1e6
        # print(edge_gates.unique().size(0)/edge_gates.size(0))
        expert_outputs = []
        ...
        # print("theory:",(edge_end_indices-edge_start_indices).sum()/num_edges_per_node.sum())
