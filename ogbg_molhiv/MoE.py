...
class MoE(nn.Module):
    ...
    def __init__(self, input_size,hidden_size, num_experts,nlayers,activation,k_list,expert_select ,noisy_gating=True, coef=1e-2,edge_dim=1,lam=1e-1):
        ...
        self.num_experts = num_experts
        # instantiate experts
        self.experts = nn.ModuleList([SpLearner(nlayers=nlayers, in_dim=input_size*2+edge_dim, hidden=hidden_size, activation=activation, k=k) for k in k_list])
    ...
    def forward(self, x, edge_index, temp, edge_attr=None, training=None):
        ...
        # print('moe theory:',k_edges_per_node.sum()/num_edges_per_node.sum())
