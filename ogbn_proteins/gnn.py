...
class DeeperGCN(torch.nn.Module):
    ...
    def forward(self, x, node_index, edge_index, edge_attr,edge_mask=None):
        ...
        if self.block == 'res+':
            ...
            if self.checkpoint_grad:
                for layer in range(1, self.num_layers):
                    ...
                    if layer % self.ckp_k != 0:
                        ...
                        h = self.gcns[layer](h2, edge_index, edge_emb,edge_mask) + h
            ...
            h = F.relu(self.layer_norms[0](self.gcns[0](h, edge_index, edge_emb,edge_mask)))
            h = F.dropout(h, p=self.dropout, training=self.training)

            for layer in range(1, self.num_layers):
                h1 = self.gcns[layer](h, edge_index, edge_emb,edge_mask)
