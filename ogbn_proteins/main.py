...
def main():
    ...
    model = MoG(dataset.x.size(1), dataset.num_tasks, args, device).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_loss = train(data, dataset, model, optimizer, criterion,temp, device,args.use_topo)
        logging.info('Epoch {}, training loss {:.4f}'.format(epoch, epoch_loss))

        result = multi_evaluate(valid_data_list, dataset, model, evaluator,temp, device,args.use_topo)
